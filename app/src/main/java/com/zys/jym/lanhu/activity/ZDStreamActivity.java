...
public class ZDStreamActivity extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener {
    String TAG="TAG--ZDStreamActivity";
    Toolbar index_toolbar;
    SwipeRefreshLayout srf_ly;
    ListView lv_zdstream;
    ...
    private void initViews() {
        ...
        lv_zdstream= (ListView) findViewById(R.id.lv_stream);
        initListViewScroll();
    }
...
//                    .addParams("login_token", getApplicationContext().getUser().getLogin_token())
                    ...
                    .execute(new StreamCallback() {
                        ...
                        public void onError(Call call, Exception e) {
                            ...
                            MyUtils.showToast(ZDStreamActivity.this, "连接服务器失败，请稍后再试");
                        }
                            ...
                            if (mData.getErrcode() == 1) {
                                if (mData.getData().getLogList().size() != 0) {
                                    ...
                                    if (mAdapter != null) {
                                        ...
                                        mAdapter = new StreamLvAdapter(ZDStreamActivity.this, mHBStreamlist, 2);
                                        lv_zdstream.setAdapter(mAdapter);
                                    }
                                } else {
                                    MyUtils.Loge(TAG, "没有更多数据");
                                    if (page == 1) {
                                        lv_zdstream.setVisibility(View.INVISIBLE);
//                                    MyUtils.showToast(ZDStreamActivity.this,mData.getErrmsg());
                                    }
                                    page--;
                                    if (page < 1) {
                                        page = 1;
...
//                            MyUtils.showToast(ZDStreamActivity.this, mData.getErrmsg());
    ...
    private void initListViewScroll() {
        lv_zdstream.setOnScrollListener(new AbsListView.OnScrollListener() {
            ...
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                ...
                switch (scrollState) {
                    ...
                    case AbsListView.OnScrollListener.SCROLL_STATE_IDLE:// 是当屏幕停止滚动时
                        scrollFlag = false;
                        // 判断滚动到底部
                        if (lv_zdstream.getLastVisiblePosition() == (lv_zdstream.getCount() - 1)) {
                            page++;
                            getData(page,false);
                        }
//                        // 判断滚动到顶部
...
//                        }
            ...
            public void onScroll(AbsListView view, int firstVisibleItem,
                ...
                if (scrollFlag
                        && ScreenUtil.getScreenViewBottomHeight(lv_zdstream) >= ScreenUtil
                        .getScreenHeight(ZDStreamActivity.this)) {
